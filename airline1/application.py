...
@app.route("/")
def index():
    flights = db.execute("SELECT * FROM flights_vw").fetchall()
...
def flight(flight_id):
    ...
    # Make sure the flight exits.
    flight = db.execute("SELECT * FROM flights_vw WHERE id = :id", {"id": flight_id}).fetchone()
    if flight is None:
        return render_template("error.html", message="No such flight with that id.")
